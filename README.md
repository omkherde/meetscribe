...
Point `notes.inbox` in the config at a synced folder (e.g. iCloud Drive) that your tablet exports into, then `meetscribe notes` sweeps it: each file is routed by filename, OCR'd, filed, and removed from the inbox. Routing is forgiving — a configured subject name appearing anywhere in the filename wins (so naming your notebooks after your subjects means default export names just work), and private subjects go to `private_vault`. Files matching no configured subject self-organize into ad-hoc topic folders: `Sociology.pdf` files under `Handwritten/Sociology/`, and `DSP - fourier basics.pdf` under `Handwritten/DSP/` titled "fourier basics" — no configuration needed for topics that aren't formal subjects.

Exports come in two flavors, chosen by the filename:

- **No date → living notebook document.** Export the *whole notebook* every time (no page selecting); each export replaces the previous pair, so `Handwritten/Sociology/Sociology.md + .pdf` is always the complete current notebook and redundant copies never accumulate.
- **`YYYY-MM-DD` in the name → dated snapshot.** A frozen record filed alongside the living doc (`2026-09-02 sleep.md`), never overwritten — for the lectures you want as standalone notes. `meetscribe notes <file> --subject X` handles one-offs.
